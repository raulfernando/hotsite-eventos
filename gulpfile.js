...
var project = 'EditoraSegmento';

var folderDest = "./themes/" + project;
...
gulp.task('images-deploy', function() {
    gulp.src(['app/images/**/*', '!app/images/README'])
        //prevent pipe breaking caused by errors from gulp plugins
        .pipe(plumber())
        .pipe(gulp.dest(folderDest+'/images'));
});
...
gulp.task('scripts-deploy', function() {
    ...
    return gulp.src(['app/scripts/src/_includes/**/*.js', 'app/scripts/src/**/*.js'])
                ...
                .pipe(uglify())
                //where we will store our finalized, compressed script
                .pipe(gulp.dest(folderDest+'/scripts'));
});
...
gulp.task('html-deploy', function() {
    ...
    gulp.src('app/fonts/**/*')
        //prevent pipe breaking caused by errors from gulp plugins
        .pipe(plumber())
        .pipe(gulp.dest(folderDest+'/fonts'));
...
//cleans our themes directory in case things got deleted
...
gulp.task('scaffold', function() {
  return shell.task([
      'mkdir '+folderDest,
      'mkdir '+folderDest+'/fonts',
      'mkdir '+folderDest+'/images',
      'mkdir '+folderDest+'/scripts',
      'mkdir '+folderDest+'/styles'
    ]
  );
});
